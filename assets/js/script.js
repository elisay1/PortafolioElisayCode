/**
 * ELISAY CODE - Portfolio JavaScript
 * Implementación moderna con animaciones avanzadas y efectos visuales
 */
...
function initApp() {
  ...
  initHoverEffects();
  init3DEffects();// Obtener el año actual para el copyright
  updateCopyrightYear();
}
...
function initProjectSearch() {
  ...
  // Mejorar estética del campo de búsqueda
  const searchContainer = searchInput.parentElement;
  const searchIcon = searchContainer.querySelector('i');
  
  searchInput.addEventListener('focus', () => {
    searchContainer.style.boxShadow = '0 0 0 2px var(--color-cyan-600), 0 0 10px rgba(1, 164, 247, 0.3)';
    searchIcon.style.color = 'var(--color-cyan-600)';
  });
  ...
  searchInput.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      searchInput.value = '';
      // Disparar el evento input para actualizar los resultados
      searchInput.dispatchEvent(new Event('input'));
      // Quitar focus
      searchInput.blur();
    }
  ...
  // Añadir animación de autofocus al cargar la página
  setTimeout(() => {
    const searchPlaceholder = document.createElement('div');
    searchPlaceholder.className = 'search-placeholder';
    searchPlaceholder.textContent = 'Busca entre mis proyectos...';
    searchPlaceholder.style.position = 'absolute';
    searchPlaceholder.style.top = '50%';
    searchPlaceholder.style.left = '50px';
    searchPlaceholder.style.transform = 'translateY(-50%)';
    searchPlaceholder.style.color = 'var(--color-text-tertiary)';
    searchPlaceholder.style.pointerEvents = 'none';
    searchPlaceholder.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
    
    searchContainer.appendChild(searchPlaceholder);
    ...
    searchPlaceholder.appendChild(cursor);
    ...
    // Hacer desaparecer el placeholder cuando se hace focus
    searchInput.addEventListener('focus', () => {
      searchPlaceholder.style.opacity = '0';
      searchPlaceholder.style.transform = 'translateY(-50%) translateX(-20px)';
    });
    
    // Mostrar el placeholder cuando se quita el focus y no hay texto
    searchInput.addEventListener('blur', () => {
...
function initAuthModals() {
  ...
  if (switchToLogin) {
    switchToLogin.addEventListener('click', (e) => {
      ...
      const registerModalElement = registerModal.querySelector('.modal');
      registerModalElement.style.transform = 'translateX(100%)';
      registerModalElement.style.opacity = '0';setTimeout(() => {
